...
parser = argparse.ArgumentParser(description='CS_BSQ PyTorch ImageNet Training')
...
parser.add_argument('--save', type=str, default='train_result/0921/IMG_T4N6*_lm0001', help='path for saving trained models')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # prune int before last test
        if epoch == args.epochs-1:
            for m in model.module.mask_modules:
                # logger.info("prune int before last test......")
                m.mask= Parameter(torch.where(m.mask >= 0.5, torch.full_like(m.mask, 1), m.mask))
                m.mask= Parameter(torch.where(m.mask < 0.5, torch.full_like(m.mask, 0), m.mask))
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    if epoch > 0: model.module.temp = temp_increase**epoch
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # for analyze only
        masks_weight = [m.mask_weight for m in model.module.mask_modules]
        masks = [m.mask for m in model.module.mask_modules]
...
def save_checkpoint(state, is_best, filename):
    file_name = os.path.join(*[filename, 'checkpoint.pth.tar'])
